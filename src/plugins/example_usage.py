...
from voice_assistant.infrastructure.container.service_containers import (
    create_asr_service_container,
    create_nlu_service_container,
    create_tts_service_container,
    create_dialog_service_container,
    create_gateway_service_container,
    create_notification_service_container,
    create_storage_service_container
)
...
async def example_usage():
    ...
    # Для примера будем использовать контейнеры конкретных сервисов
    print("=== Пример использования ASR сервиса ===")
    asr_container_wrapper = create_asr_service_container()
    asr_container = asr_container_wrapper.get_container()
    ...
    try:
        ...
        # NLU сервис
        nlu_container_wrapper = create_nlu_service_container()
        nlu_container = nlu_container_wrapper.get_container()
        nlu_service = nlu_container.resolve(NLUPlugin)
        print(f"NLU Service: {type(nlu_service).__name__}")

        # TTS сервис
        tts_container_wrapper = create_tts_service_container()
        tts_container = tts_container_wrapper.get_container()
        tts_service = tts_container.resolve(TTSPlugin)
...
async def example_direct_component_access():
    ...
    # Используем ASR контейнер для демонстрации
    asr_container_wrapper = create_asr_service_container()
    container = asr_container_wrapper.get_container()
