...
def initialize_asr_plugins():
    """Initialize ASR plugins."""
    primary_asr = None
    fallback_asr = None
    
    # Try to load primary ASR (Vosk)
    try:
        ...
        primary_asr = VoskASRPlugin()
        logger.info("Vosk ASR plugin initialized")
        
    except Exception as e:
        logger.warning(f"Failed to initialize Vosk ASR: {e}")

    if not primary_asr and not fallback_asr:
        logger.warning("No ASR plugins available, using mock")
...
async def serve():
    ...
    # Initialize plugins
    primary_asr, fallback_asr = initialize_asr_plugins()
